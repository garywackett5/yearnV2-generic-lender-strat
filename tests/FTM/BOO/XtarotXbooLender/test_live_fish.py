...
def test_normal_activity(
    ...
    for i in range(2):
        ...
        strState = vault.strategies(strategy)
        totalReturns = strState[7]-oldTotalReturns
    ...
    print(t1.events)
    assert strategy.estimatedTotalAssets() <= 1e17


def test_withdrawal_works(
    wftm,
    boo,
    chain,
    whale,
    ...
    whale_balance = boo.balanceOf(whale)
    vault.deposit(10_000 *1e18, {'from': whale})
    ...
    assert boo.balanceOf(vault) + boo.balanceOf(strategy) < 10_000*1e18
    vault.withdraw({'from': whale})
    assert boo.balanceOf(whale) > whale_balance
